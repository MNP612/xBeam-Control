...
class Application(tk.Frame):
    def __init__(self, master=None):
        ...
        try:
            self.cap2 = IC.TIS_CAM()
            self.cap2.open('DFK 72BUC02 28910508')
            self.cap2.SetVideoFormat('RGB32 (640x480)')
            self.cap2.SetFrameRate(25.0)
            self.cap2.StartLive(0)
            self.show_cam2_TIS()
        except:
            image = Image.new('RGB', (640, 480), (0, 0, 255))
            imgtk = ImageTk.PhotoImage(image=image)
            self.cam2_label.imgtk = imgtk
    ...
    def show_cam2_TIS(self):
        self.cap2.SnapImage()
        #capture frame-by-frame
        frame = self.cap2.GetImage()
        #our operations on  the frame came here
        cv2image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        #overlay crossair
        cv2image[int(HEIGHT/2),:int(WIDTH/2)-10,:]= [(255,0,0)]
        ...
        #display the resulting frame
        cv2.putText(cv2image, 'CAM 2: Beam Axis', (10,25), 4, .8, (0, 0, 0), thickness=2)
        cv2.putText(cv2image, 'CAM 2: Beam Axis', (10,25), 4, .8, (255, 255, 255), thickness=1)
        image = PIL.Image.fromarray(cv2image)
        imgtk = ImageTk.PhotoImage(image=image)
        self.cam2_label.imgtk = imgtk
        self.cam2_label.configure(image=self.cam2_label.imgtk)
        self.cam2_label.after(10, self.show_cam2_TIS)
